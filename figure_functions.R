...
updatePosDemo <- function(data, param) {
  ...
  ds %>% ggplot(aes(x=tno, y=pos, fill=weight)) + geom_tile() + geom_point(aes(y=tpos)) + 
    geom_point(aes(y=d1pos), shape=2) + geom_point(aes(y=d2pos), shape=2) + 
    geom_point(aes(y=d3pos), shape=2) + theme_classic() + labs(x="Trial", y="Position", 
                                                               fill="Weight") +  
  ...
  fig_line <- filter(ds, tpos==pos) %>% ggplot(aes(x=tno, y=weight*param$mu, group=1)) + 
    geom_point() + geom_line()+ labs(x="Trial", y="Rate") + theme(text = element_text(size=15)) +
    geom_hline(aes(yintercept=param$mu), linetype=2)
...
updateColDemo <- function(data, param) {
  ...
  ds %>% ggplot(aes(x=tno, y=col, fill=weight)) + geom_tile() + geom_point(aes(y=tcol)) + 
    geom_point(aes(y=dcol), shape=2) + theme_classic() + labs(x="Trial", y="Color", fill="Weight") +  
    scale_fill_gradient(low = "white", high = "gray25") + theme(text = element_text(size=15)) -> fig_hmap
